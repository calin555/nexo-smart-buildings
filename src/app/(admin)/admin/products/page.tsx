...
function formatPrice(priceFrom: number): string {
  if (priceFrom === 0) return "La cerere";
            ...
            <table className="w-full min-w-[720px] text-left text-sm">
              ...
              <tbody className="divide-y divide-slate/10">
                {products.map((product) => (
                  <tr key={product.id} className="hover:bg-cloud/60">
                    <td className="px-5 py-4">
                      <p className="font-medium text-ink">{product.name}</p>
                      <p className="mt-1 text-xs text-slate">{product.brand}</p>
                    </td>
                    <td className="px-5 py-4 text-slate">{product.category}</td>
                    <td className="px-5 py-4 font-medium">{formatPrice(product.priceFrom)}</td>
                    <td className="px-5 py-4">
                      {product.active ? (
